...
import { howls, howlThreads, type User } from "../schema";
...
export const createHowlThread = async (content: string, user: User) => {
	const userId = user.id;
	const insertedThread = await db
		.insert(howlThreads)
		.values({ userId })
		.returning({ id: howlThreads.id });
	await db
		.insert(howls)
		.values({ content, userId, threadId: insertedThread[0].id });
