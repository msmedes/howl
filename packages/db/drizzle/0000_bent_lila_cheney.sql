...
CREATE TABLE "howls" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "howls_agentId_unique" UNIQUE("agent_id")
);
--> statement-breakpoint
...
CREATE TABLE "users" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "users_agentId_unique" UNIQUE("agent_id"),
...
ALTER TABLE "agent_threads" ADD CONSTRAINT "agent_threads_agent_id_users_id_fk" FOREIGN KEY ("agent_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
